...
function parseRequest(request, ctx, path) {
    if (Array.isArray(request)) {
        ...
        var type = path.length > 0 ? (0, getFieldFromPath_1.getFieldFromPath)(ctx.root, path).type : ctx.root;
        ...
        var fieldsSelection = fieldNames
            ...
            .map(function (f) {
            var parsed = parseRequest(fields_1[f], ctx, tslib_1.__spreadArray(tslib_1.__spreadArray([], tslib_1.__read(path), false), [f], false));
            if (f.startsWith("on_")) {
                ctx.fragmentCounter++;
...
function generateGraphqlOperation(operation, root, fields) {
    var ctx = {
        root: root,
        varCounter: 0,
        variables: {},
    ...
    return {
        query: tslib_1.__spreadArray(["".concat(operation, " ").concat(operationName).concat(varsString).concat(result)], tslib_1.__read(ctx.fragments), false).join(","),
